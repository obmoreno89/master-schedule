...
import LineRateCustomCreate from './LineRateCustomCreate';

function LineRateCustomCreatePanel({
  ...
  //     if (
  //       !Line RateCustomCreateOpenPanel ||
  //       panelContent.current.contains(target) ||
  //       closeBtn.current.contains(target)
  //     )
  //       return;
  //     setLine RateCustomCreateOpenPanel(false);
  //   };
  ...
  useEffect(() => {
    const keyHandler = ({ keyCode }) => {
      if (!LineRateCustomCreateOpenPanel || keyCode !== 27) return;
  ...
  return (
    <>
      <Transition
        ...
        show={LineRateCustomCreateOpenPanel}
      ...
      >
        <div
          ...
          className={` absolute inset-0 sm:left-auto z-40 transform shadow-xl transition-transform duration-200 ease-in-out ${
            LineRateCustomCreateOpenPanel ? 'translate-x-' : 'translate-x-full'
          }`}
        >
          <div className='top-16 bg-white overflow-x-hidden overflow-y-auto no-scrollbar shrink-0 border-l border-slate-200 w-full sm:w-[390px] h-screen'>
            <div className='flex justify-between'>
              ...
              <button
                ref={closeBtn}
                onClick={() => setLineRateCustomCreateOpenPanel(false)}
...
export default LineRateCustomCreatePanel;
