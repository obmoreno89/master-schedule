...
import {
  selectBaseCapabilitiesFile,
  selectToggleBaseCapabilitiesFile,
  setToggleBaseCapabilitiesFile,
} from '../../store/slice/BaseCapabilitiesSlice';

function ModalBaseCapabilitiesExport({
  modalBaseCapabilitiesExportOpen,
  setModalBaseCapabilitiesExportOpen,
}) {
  const dispatch = useDispatch();
  const fileDownload = useSelector(selectBaseCapabilitiesFile);
  const handleFile = useSelector(selectToggleBaseCapabilitiesFile);

  return (
    <ModalBlank
      id='success-modal'
      modalOpen={modalBaseCapabilitiesExportOpen}
      setModalOpen={setModalBaseCapabilitiesExportOpen}
    >
          ...
          {handleFile && (
            <div className='mb-8 flex justify-between items-center'>
              <div className='text-lg font-bold text-slate-800'>
                Reporte Capacidades Base
              </div>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setModalBaseCapabilitiesExportOpen(false);
                  dispatch(setToggleBaseCapabilitiesFile(false));
                }}
              >
                <svg
...
export default ModalBaseCapabilitiesExport;
