...
import BaseCapabilitiesCreate from './BaseCapabilitiesCreate';

function BaseCapabilitiesPanel({
  ...
  useEffect(() => {
    const keyHandler = ({ keyCode }) => {
      if (!baseCapabilitiesPanelOpen || keyCode !== 27) return;
  ...
  return (
    <>
      <Transition
        ...
        show={baseCapabilitiesPanelOpen}
      ...
      >
        <div
          ...
          className={` absolute inset-0 sm:left-auto z-40 transform shadow-xl transition-transform duration-200 ease-in-out ${
            baseCapabilitiesPanelOpen ? 'translate-x-' : 'translate-x-full'
          }`}
        >
          <div className='top-16 bg-white overflow-x-hidden overflow-y-auto no-scrollbar shrink-0 border-l border-slate-200 w-full sm:w-[390px] h-screen'>
            <div className='flex justify-between'>
              ...
              <button
                ref={closeBtn}
                onClick={() => setBaseCapabilitiesOpenPanel(false)}
...
export default BaseCapabilitiesPanel;
