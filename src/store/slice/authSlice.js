...
export const sendData = (data, navigate) => (dispatch) => {
  ...
  axios
    .post('http://3.88.215.84/api/auth/login/', data)
    .then((response) => {
...
export const logoutUser = (navigate) => () => {
  ...
  axios
    .post('http://3.88.215.84/api/auth/logout/', email, {
...
export const emailSend = (data, navigate) => (dispatch) => {
  dispatch(setLoading(true));
  axios
    .post('http://3.88.215.84/api/auth/password-reset/send-code', data)
    .then((response) => {
      dispatch(setLoading(false));
      if (response.data.status_code === 200) {
...
export const codeSend = (data, navigate) => (dispatch) => {
  ...
  axios
    .post('http://3.88.215.84/api/auth/password-reset/verify-code', json)
    .then((response) => {
      dispatch(setLoading(false));
      if (response.data.status_code === 202) {
        sessionStorage.setItem('token', response.data.token);
...
export const confirmNewPass = (data, navigate) => (dispatch) => {
  ...
  axios
    .post('http://3.88.215.84/api/auth/password-reset/confirmation', json, {
