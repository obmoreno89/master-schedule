...
import { revertCapEdit } from '../../store/slice/capabilitiesSlice';
import Transition from '../../utils/Transition';
import CapabilitiesEdit from './CapabilitiesEdit';

function CapabilitiesEditPanel({
  ...
  useEffect(() => {
    const keyHandler = ({ keyCode }) => {
      if (!capabilitiesEditOpen || keyCode !== 27) return;
      ...
      >
        <div
          ...
          className={`absolute inset-0 sm:left-auto z-40 transform shadow-xl transition-transform duration-200 ease-in-out ${
            capabilitiesEditOpen ? 'translate-x-' : 'translate-x-full'
          }`}
        >
          <div className='top-16 bg-white overflow-x-hidden overflow-y-auto no-scrollbar shrink-0 border-l border-slate-200 w-full sm:w-[390px] h-screen'>
            <div className='flex justify-between'>
              <h2 className='mt-4 ml-4 font-bold text-black text-2xl'>
                Editar capacidad
              </h2>
              <button
                ref={closeBtn}
...
export default CapabilitiesEditPanel;
